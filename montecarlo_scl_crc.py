...
def crcFunc(msg):
    GF = galois.GF(2)
    msg = list(msg) + [0] * (len(crcPoly)-1)
    crcFn = galois.Poly(crcPoly, field=GF)
    msg_poly = galois.Poly(msg, field=GF)

    remainder = msg_poly % crcFn
    remainder_coeff = remainder.coeffs
    if(remainder == 0):
        return msg
    else:
        pL = max(0, len(crcFn) - 1 - len(remainder_coeff))

        act_rem = np.concatenate((remainder_coeff[::-1], np.zeros(pL, dtype=int)))
        for i in range(len(crcFn) -1):
            msg[-(1+i)] = int(act_rem[i]) 
        
        return msg

def crccheck(decoded_lists):
    remainder_zero =[]
    remainder_non_zero= []
    GF = galois.GF(2)
    crcFn = galois.Poly(crcPoly, field=GF)

    for i in range(len(decoded_lists)):
        msg_poly = galois.Poly(decoded_lists[i], field=GF)
        r = msg_poly%crcFn
        if (r == 0):
            remainder_zero.append(decoded_lists[i])
        else:
            remainder_non_zero.append(decoded_lists[i])
    
    
    if(len(remainder_zero) == 0):
        return remainder_non_zero
    else:
...
crcPoly = [1,0,0,1,1] #x^4 + x + 1 

N = [16,32,64,128] #Total bits
A = [8,16,32,64] #actual msg bits
Nsim = 1000#Total number of simulation for a given EbN0 value

for z in range(len(N)):
    n = int(mt.log2(N[z]))+1   #maximum depth of tree
    K = A[z] + len(crcPoly) - 1     #actual msg bits
    Rate = A[z]/N[z]
    ...
    F = Q[: N[z]-K]    #frozen bits
    NF = Q[N[z]-K :]   #non-frozen bits
    ...
    for vals in range(len(EbN0)):
        ...
        for times in range(0,Nsim):
            ...
            newmsg = np.random.randint(2, size=A[z])
            ...
            final_ans_list = crccheck(ans_decodedlist)
            count = 0
            for i in range(len(final_ans_list[0])):
    ...
    plt.figure(1)
    plt.yscale('log')
    plt.plot(EbN0dB,Ber_scl,label='simulated', marker='o', linestyle='-', color='blue',linewidth=2)
    plt.plot(EbN0dB,theoritical,label='Theoritical', marker='o', linestyle='-', color='red',linewidth=2)
    plt.xlabel('Eb/N0 (dB)')  # Label x-axis
    plt.ylabel('Bit Error Rate (BER for SCL)')  # Label y-axis
    plt.title(f'Bit Error Rate (SCL) vs. Eb/N0 (N={N}, A={A}, Rate={Rate}, Nsim={Nsim})')
    plt.grid(True, which='both', linestyle='--', linewidth=0.5)  # Add gridlines
    plt.legend(loc='upper left')  # Move legend to upper right corner
    plt.legend()


    plt.figure(2)
    plt.yscale('log')
    plt.plot(EbN0dB,blker_scl,label = 'Block error rate simulated', marker='o', linestyle='-', color='blue',linewidth=2)
    plt.xlabel('Eb/N0 (dB)')  # Label x-axis
    plt.ylabel('Block Error Rate (BLKER for SCL)')  # Label y-axis
    plt.title(f'Block Error Rate (SCL) vs. Eb/N0 (N={N}, A={A}, Rate={Rate}, Nsim={Nsim})')
    plt.grid(True, which='both', linestyle='--', linewidth=0.5)  # Add gridlines
    plt.legend(loc='upper left')  # Move legend to upper right corner
    plt.legend()
    ...
    plt.figure(3)
    plt.yscale('log')
    plt.plot(EbN0dB,success_scl,label = f'(N={N[z]}, K={A[z]})', linestyle='-',linewidth=2)
    plt.xlabel('Eb/N0 (dB)')  # Label x-axis
